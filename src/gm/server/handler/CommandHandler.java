        ...
        switch (command) {
            case 0: {//notice
                for (World world : Server.getInstance().getWorlds()) {
                    world.broadcastPacket(MaplePacketCreator.broadcastMsg(0, slea.readMapleAsciiString()));
                }
                break;
            }
            ...
            case 7: {
                Server.getInstance().shutdownThread(false).run();
            }
            case 8: {
                Server.getInstance().shutdownThread(true).run();
            }
        }
    }
