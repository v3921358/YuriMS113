...
public abstract class MapleEvent {
    ...
    public void incrementPlayerCount() {
        playerCount++;
        if (playerCount == 250) {
            setEvent(Channel.getInstance(channel), true);
        }
    }
    ...
    public void onMapLoad(MapleCharacter chr) { //most dont do shit here
        if (GameConstants.isEventMap(chr.getMapId()) && FieldLimit.Event.check(chr.getMap().getFieldLimit()) && FieldLimit.Event2.check(chr.getMap().getFieldLimit())) {
    ...
    public static void setEvent(final Channel cserv, final boolean auto) {
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            e.broadcast(MaplePacketCreator.broadcastMsg(0, "Entries for the event are now closed!"));
                            e.broadcast(MaplePacketCreator.broadcastMsg(0, "The event will start in 30 seconds!"));
    ...
    public static void mapLoad(final MapleCharacter chr, final int channel) {
        ...
        for (MapleEventType t : MapleEventType.values()) {
            final MapleEvent e = Channel.getInstance(channel).getEvent(t);
            if (e.isRunning) {
    ...
    public static String scheduleEvent(final MapleEventType event, final Channel cserv) {
        ...
        for (int i : cserv.getEvent(event).type.mapids) {
            if (cserv.getMapFactory().getMap(i).getCharacters().size() > 0) {
        ...
        World.Broadcast.broadcastMessage(MaplePacketCreator.broadcastMsg(0, "Hello " + ServerConstants.WORLD_NAMES[cserv.getId()] + "! Let's play a " + StringUtil.makeEnumHumanReadable(event.name()) + " event in channel " + cserv.getId() + "! Change to channel " + cserv.getId() + " and use @event command!"));
        return "";
    }
