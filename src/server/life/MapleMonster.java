...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            ...
            for (Item cashItem : attacker.getInventory(MapleInventoryType.CASH)) {
                int itemId = cashItem.getItemId();
                if (ItemConstants.isExpRateCard(itemId)) {
                    if ( ( ItemConstants.getExpRateCardTime(itemId).getValue() & RateWorker.getTime().getValue() ) != 0) {
                        personalExp *= 2;
                        break;
                    }
                }
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
        ...
        if (this.getController() != null) { // this can/should only happen when a hidden gm attacks the monster
            getController().getClient().announce(MonsterPacket.stopControllingMonster(this.getObjectId()));
    ...
    public void switchController(MapleCharacter newController, boolean immediateAggro) {
        ...
        if (controllers != null) {
            ...
            controllers.getClient().announce(MonsterPacket.stopControllingMonster(getObjectId()));
        }
    ...
    public void sendSpawnData(MapleClient c) {
        ...
        if (isFake()) {
            c.announce(MonsterPacket.spawnFakeMonster(this, 0));
        } else {
            c.announce(MonsterPacket.spawnMonster(this, false));
        }
        if (stati.size() > 0) {
    ...
    public void updateLastDropTime(long currentTimeMillis) {
        this.lastdropTime = currentTimeMillis;
    }
