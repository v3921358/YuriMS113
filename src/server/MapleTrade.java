...
public class MapleTrade {
    ...
    private void complete1() {
        ...
        partner.getChr().message("完成了與 " + chr.getName()+ " 的交易。");
    }
    ...
    public void setMeso(int meso) {
        if (locked) {
            throw new RuntimeException("交易被鎖定。");
        }
        if (meso < 0) {
    ...
    public static void completeTrade(MapleCharacter c) {
        ...
        if (partner.isLocked()) {
            ...
            if (!local.fitsInInventory() || !partner.fitsInInventory()) {
                cancelTrade(c);
                c.message("沒有足夠的道具欄空間來完成交易。");
                partner.getChr().message("沒有足夠的道具欄空間來完成交易。");
                return;
            }
    ...
    public static void declineTrade(MapleCharacter c) {
        ...
        if (trade != null) {
            if (trade.getPartner() != null) {
                ...
                other.setTrade(null);
                other.message(c.getName() + " 拒絕您的交易請求。");
            }
            trade.cancel();
            c.setTrade(null);
        }
