...
public class MapleStatEffect {
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case Noblesse.SPACE_DASH:
                    statups.add(new Pair<>(MapleBuffStat.DASH_JUMP, Integer.valueOf(ret.x)));
                    statups.add(new Pair<>(MapleBuffStat.DASH_SPEED, Integer.valueOf(ret.y)));
                    break;
    ...
    private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
        ...
        if (applyfrom.getMp() < getMpCon()) {
            AutobanFactory.MPCON.addPoint(applyfrom.getAutobanManager(), "偵測到技能：:" + sourceid + "魔力遭修改; 角色魔力: " + applyto.getMp() + " 需要魔力: " + getMpCon());
        }
        if (hpchange != 0) {
        ...
        if (moveTo != -1) {
            if (applyto.getMap().getReturnMapId() != applyto.getMapId()) {
                ...
                if (moveTo == 999999999) {
                    ...
                    System.err.println(applyto.getMapId());
                    System.err.println(target.getId());
                    if (targetid != 60 && applyto.getMapId() / 10000000 != 61 && targetid != applyto.getMapId() / 10000000 && targetid != 21 && targetid != 20 && targetid != 12) {
                        return false;
                    }
                }
        ...
        if (isMagicDoor() && !FieldLimit.MysticDoor.check(applyto.getMap().getFieldLimit())) { // Magic Door
            Point doorPosition = new Point(applyto.getPosition());
            MapleDoor door = new MapleDoor(applyto, doorPosition, sourceid);
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
        ...
        if (localstatups.size() > 0) {
            ...
            if (isDash()) {
                ...
                mbuff = MaplePacketCreator.showMonsterRiding(applyto.getId(), localstatups, givemount.getItemId(), sourceid);
    ...
    private boolean isDs() {
        return skill && (sourceid == Rogue.DARK_SIGHT || sourceid == WindArcher.WIND_WALK || sourceid == NightWalker.DARK_SIGHT || sourceid == GM.HIDE || sourceid == SuperGM.HIDE);
    }
