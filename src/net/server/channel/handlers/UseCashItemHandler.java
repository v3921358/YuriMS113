...
public final class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        String medal = "";
        Item medalItem = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -21);
        if (medalItem != null) {
        ...
        if (itemType == 505) { // AP/SP reset
            ...
            switch (itemId / 1000 % 10) {
                case 1: // Megaphone
                    if (player.getLevel() > 9) {
                        player.getClient().getChannelServer().broadcastPacket(MaplePacketCreator.broadcastMsg(2, medal + player.getName() + " : " + slea.readMapleAsciiString()));
                    } else {
                        player.dropMessage(1, "你需要10等以上。");
                    }
                    break;
                case 2: // Super megaphone
                    Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.broadcastMsg(3, c.getChannel(), medal + player.getName() + " : " + slea.readMapleAsciiString(), (slea.readByte() != 0)));
                    break;
                case 5: // Maple TV
                    ...
                    slea.readInt();
                    if (megassenger) {
                        Server.getInstance().broadcastMessage(c.getWorld(), MaplePacketCreator.broadcastMsg(3, c.getChannel(), medal + player.getName() + " : " + builder.toString(), ear));
                    }
            ...
            Item item = player.getInventory(MapleInventoryType.CASH).getItem(pet.getInventorypos());
            ...
            if (!vip) {
                final MapleMap targetMap = c.getChannelServer().getMapFactory().getMap(slea.readInt());
                if (targetMap != null) {
                    if ((itemId == 5041000 && c.getPlayer().isTrockMap(targetMap.getId())) || (itemId != 5041000 && c.getPlayer().isVipTrockMap(targetMap.getId()))) {
                        if (!FieldLimit.VipRock.check(c.getPlayer().getMap().getFieldLimit()) && !FieldLimit.VipRock.check(targetMap.getFieldLimit()) && c.getPlayer().getEventInstance() == null) { //Makes sure this map doesn't have a forced return map
                            c.getPlayer().changeMap(targetMap, targetMap.getPortal(0));
                        } else {
