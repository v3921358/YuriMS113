...
public final class MoveLifeHandler extends AbstractMovementPacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (toUse != null) {
            c.announce(MonsterPacket.moveMonsterResponse(objectid, moveid, monster.getMp(), aggro, toUse.getSkillId(), toUse.getSkillLevel()));
        } else {
            c.announce(MonsterPacket.moveMonsterResponse(objectid, moveid, monster.getMp(), aggro));
        }
        if (aggro) {
        ...
        if (res != null) {
            ...
            if(monster.getPosition().distanceSq(startPos) >  120000 )
            {
                AutobanFactory.MOB_VAC.addPoint(c.getPlayer().getAutobanManager(), "Mob Vac, distance:" + String.valueOf(monster.getPosition().distanceSq(startPos)));
            }
