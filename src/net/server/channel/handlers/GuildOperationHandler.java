        ...
        switch (type) {
            ...
            case 0x02:
                if (mc.getGuildId() > 0 || mc.getMapId() != 200000301) {
                    c.getPlayer().dropMessage(1, "你已經有一個公會所以不能創建.");
                    return;
                }
                ...
                if (!isGuildNameAcceptable(guildName)) {
                    c.getPlayer().dropMessage(1, "這個創建公會的名字是被禁止的.");
                    return;
                }
                int gid;
                ...
                if (gid == 0) {
                    c.announce(GuildPacket.genericGuildMessage((byte) 0x1c));
                    return;
                }
                ...
                c.getPlayer().dropMessage(1, "你成功創建了公會.");
                respawnPlayer(mc);
                break;
            case 0x05:
            ...
            case 0x06:
                ...
                if (s == 0) {
                    c.getPlayer().dropMessage(1, "你加入的工會人數達到上限。");
                    mc.setGuildId(0);
                    return;
                }
                c.announce(GuildPacket.showGuildInfo(mc));
