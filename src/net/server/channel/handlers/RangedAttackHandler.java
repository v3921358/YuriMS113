...
public final class RangedAttackHandler extends AbstractDealDamageHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (attack.skill == Buccaneer.ENERGY_ORB
                || attack.skill == ThunderBreaker.SPARK
                || attack.skill == Shadower.TAUNT
                || attack.skill == NightLord.TAUNT) {
        ...
        } else if (attack.skill == Aran.COMBO_SMASH
                || attack.skill == Aran.COMBO_PENRIL
                || attack.skill == Aran.COMBO_TEMPEST) {
            player.getMap().broadcastMessage(player, MaplePacketCreator.rangedAttack(player, attack.skill, attack.skilllevel, attack.stance, attack.numAttackedAndDamage, 0, attack.allDamage, attack.speed, attack.direction, attack.display), false);
            ...
            final Integer ShadowPartner = player.getBuffedValue(MapleBuffStat.SHADOWPARTNER);
            boolean soulArrow = player.getBuffedValue(MapleBuffStat.SOULARROW) != null;
            boolean shadowClaw = player.getBuffedValue(MapleBuffStat.SHADOW_CLAW) != null;
            int projectile = 0, visProjectile = 0;
            if (attack.AOE != 0 && player.getBuffedValue(MapleBuffStat.SOULARROW) == null && attack.skill != 4111004) {
                if (player.getInventory(MapleInventoryType.USE).getItem(attack.slot) == null) {
                    return;
                }
                projectile = player.getInventory(MapleInventoryType.USE).getItem(attack.slot).getItemId();

                if (attack.csstar > 0) {
                    if (player.getInventory(MapleInventoryType.CASH).getItem(attack.csstar) == null) {
                        return;
                    }
                    visProjectile = player.getInventory(MapleInventoryType.CASH).getItem(attack.csstar).getItemId();
                } else {
                    visProjectile = projectile;
                }
                // Handle bulletcount
                if (player.getBuffedValue(MapleBuffStat.SHADOW_CLAW) == null) {
                    int bulletConsume = bulletCount;
                    if (effect != null && effect.getBulletConsume() != 0) {
                        bulletConsume = effect.getBulletConsume() * (ShadowPartner != null ? 2 : 1);
                    }
                    if (!MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, projectile, bulletConsume, false, true)) {
                        player.dropMessage(5, "You do not have enough arrows/bullets/stars.");
                        return;
                    }
                }
            }
            ...
            if (projectile != 0 || soulArrow || attack.skill == 11101004 || attack.skill == 15111007 || attack.skill == 14101006) {
                ...
                /* visProjectile = projectile; //visible projectile sent to players
                ...
                if (soulArrow //bow, crossbow
                        || attack.skill == WindArcher.PUPPET
                        || attack.skill == 3211004
