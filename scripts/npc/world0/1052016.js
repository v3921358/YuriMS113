...
var cost = Array(1000, 800, 1200, 1000, 1200);
var costBeginner = Array(120, 100, 80, 100, 100, 120);
var selectedMap = -1;
var job;
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendNext("您好~! 墮落城市計程車. 想要往其他村莊安全又快速的移動嗎? 如果是這樣 為了優先考量滿足顧客, 請使用 #b墮落城市計程車#k 特別免費! 親切的送你到想要到達的地方");
        } else if (status == 1) {
            if (cm.getJobId()==0) {
